...
Sensor * fill_humd_solo_structs(char *name, char *unit, int number_of_sensors, unsigned long frequency, short *ids, int min_lim, int max_lim){
	...
	Sensor *humd_solo = (Sensor *) malloc (number_of_sensors*sizeof(Sensor));
	...
	if (humd_solo != NULL) {
		...
		while (counter != 0 && reads != NULL && humd_solo != NULL) {
			(*(humd_solo+j)).id = *(ids);
			(*(humd_solo+j)).sensor_type = name;
			(*(humd_solo+j)).unit_of_measurement = unit;
			(*(humd_solo+j)).frequency = frequency;
			(*(humd_solo+j)).readings_size = reads_size;
			(*(humd_solo+j)).readings = reads;
			(*(humd_solo+j)).min_limit = min_lim;
			(*(humd_solo+j)).max_limit = max_lim;
			(*(humd_solo+j)).errors = count_errors(reads, reads_size, min_lim, max_lim);
			*(ids) = *(ids) + 1;
