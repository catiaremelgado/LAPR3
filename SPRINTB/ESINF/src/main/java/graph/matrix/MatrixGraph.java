...
public class MatrixGraph<V,E> extends CommonGraph<V,E> {
    ...
    public Collection<Edge<V, E>> edges() {
        ArrayList<Edge<V, E>> edgesOfGraph=new ArrayList<>();
        for (int i = 0; i < numVerts; i++) {
            for (int j = 0; j < numVerts; j++) {
                if(!edgesOfGraph.contains(edgeMatrix[i][j]) && edgeMatrix[i][j]!=null) {
                    edgesOfGraph.add(edgeMatrix[i][j]);
                }
    ...
    public Collection<Edge<V, E>> outgoingEdges(V vert) {
        ...
        for (int i = 0; i < numVerts; i++) {
            if (edgeMatrix[vertKey][i] != null) {
                edges.add(edgeMatrix[vertKey][i]);
            }
        }
