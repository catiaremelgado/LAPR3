...
class AlgorithmsTest {
    ...
    @Test
    public void testShortestPathWithMustPass(){

        TreeMap<String, Pair<Float, Float>> coordsMap = new TreeMap<>();
        TreeMap<Location, BusinessEntity> map = StoreData.readLocations("clientes-produtores_mini_v1.csv", coordsMap);
        Graph<BusinessEntity, Integer> g = StoreData.readGraphData("distancias_mini_v1_special.csv", map, coordsMap);

        ArrayList<LinkedList<BusinessEntity>> paths = new ArrayList<>();
        ArrayList<Integer> dists = new ArrayList<>();
        ArrayList<BusinessEntity> mustpass = new ArrayList<>();

        mustpass.add(g.vertex(5)); //E3
        mustpass.add(g.vertex(6)); //P2
        mustpass.add(g.vertex(0)); //C1
        mustpass.add(g.vertex(1)); //P3

        Algorithms.shortestPathWithMustPass(g, mustpass, Integer::compare, Integer::sum, 0, paths, dists);

        assertEquals(paths.get(0).getFirst(), g.vertex(5));
        assertEquals(paths.get(1).getFirst(), g.vertex(6));
        assertEquals(paths.get(2).getFirst(), g.vertex(0));
        assertEquals(paths.get(2).getLast(), g.vertex(1));
    }
